...
Node* createPostorder(int n, int &i,int maxlimit, int pre[]){
    if(i>=n || pre[i]>maxlimit) return NULL;
    
    Node* nd= newNode(pre[i++]);
    
    nd->left= createPostorder(n,i,nd->data,pre);
    nd->right= createPostorder(n,i,maxlimit,pre);
...
Node* post_order(int pre[], int size)
{
    ...
    int i=0;
    Node* root= createPostorder(size,i, INT_MAX,pre);
    return root;
